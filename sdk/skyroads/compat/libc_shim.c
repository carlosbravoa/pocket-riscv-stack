...
/* ── OpenJazz additions (not in the Wolf3D shim) ────────────────────────────── */
#include <ctype.h>
...
#ifndef RVSTACK_PC
/* OpenJazz uses no dirent (PoP did); dropped. */
#endif

/* ── more picolibc-minimal gaps for OpenJazz ────────────────────────────────── */
#include <stdio.h>
...
/* fscanf: the only files read with it (OpenJazz.ini, names.txt) are declined by
...
/* fwrite for the C++ runtime lives in rv_shim.c — SkyRoads has no pak-stdio
 * layer (assets go through pakfs_data + the upstream *_bytes loaders). */
