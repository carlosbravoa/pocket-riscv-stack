...
#include "stb_vorbis.c"        // header-only: prototypes only (impl dropped; see
                               // compat/vorbis_stub.c — Ogg music unsupported)

#ifdef POP_RVSTACK
// RVSTACK: use the compat shadow SDL (compat/SDL.h) — the console has no real
// SDL2. SDL_image (icon/PNG-override path) is dropped; its IMG_* calls become
// no-ops in compat/SDL.h.
# include "SDL.h"
