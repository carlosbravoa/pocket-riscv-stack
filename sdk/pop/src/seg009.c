...
#ifdef POP_RVSTACK
const int digi_samplerate = 48000;   // RVSTACK: match the HAL stream (48 kHz);
                                     // sounds resample to this, callback emits it
#else
...
void update_screen() {
	init_scaling();          // RVSTACK: hoisted (creates overlay/merged surfaces
	                         // draw_overlay needs; textures are no-op stubs here)
	draw_overlay();
	SDL_Surface* surface = get_final_surface();
#ifdef POP_RVSTACK
	// RVSTACK: the console has no SDL renderer/texture. Present PoP's final
	// truecolor surface as 8bpp indices through the hardware palette —
	// pop_sdl.c's pop_present() quantizes (reverse-LUT) + blitter-presents.
	extern rgb_type palette[256];
	extern void pop_present(const unsigned char *rgb, int pitch, int w, int h,
	                        const unsigned char pal[256][3]);
	static unsigned char pal8[256][3];
	for (int i = 0; i < 256; i++) {          // PoP palette is 6-bit VGA -> 8-bit
		pal8[i][0] = (unsigned char)(palette[i].r << 2);
		pal8[i][1] = (unsigned char)(palette[i].g << 2);
		pal8[i][2] = (unsigned char)(palette[i].b << 2);
	}
	pop_present((const unsigned char*)surface->pixels, surface->pitch,
	            surface->w, surface->h, pal8);
	return;
#else
	if (scaling_type == 1) {
