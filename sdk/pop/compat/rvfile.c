/*
 * rvfile.c — FILE-stream shim for the SDLPoP riscv-stack port.
 *
 * SDLPoP does classic stdio on ~8 data files plus config/savegames; the
 ...
 * pakfs module directly (pop.pak is ~4 MB — it fits the HAL's
 ...
 * of a game's regions share the 32 KB save window. A PoP savestate is only a
 * few KB (level + kid + guards), so config + one quicksave fit the window
 * comfortably — see PORTING.md. */
typedef struct { const char *name; uint32_t cap; } upersist_t;
static const upersist_t upersist[] = {
	{ "sdlpop.cfg",    2048 },   /* menu/options config */
	{ "quicksave.sav", 16384 },  /* PoP savestate (level + kid + guards) */
};
