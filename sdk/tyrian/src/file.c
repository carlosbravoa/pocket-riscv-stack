...
{
#ifndef OF_PC
	// On the Pocket the data lives in the in-memory pack; no directory probe.
	return OF_DATA_DIR;
#else
...
{
#ifndef OF_PC
	// Pocket: data files come from the in-memory pack; user files (config,
	// saves) from nonvolatile save slots. See of_files.c.
	if (of_is_data_dir(dir))
		return of_pak_open(file);
	if (of_is_user_dir(dir))
		return of_user_open(file, mode);
#endif
