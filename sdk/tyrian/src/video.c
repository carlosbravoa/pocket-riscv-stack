...
/* RVSTACK: window/renderer/texture machinery is desktop-only; the Pocket
 * present path pushes the indexed frame straight to hardware. */
#ifdef OF_PC
...
#ifndef OF_PC
	/* RVSTACK: SDL_InitSubSystem(SDL_INIT_VIDEO) above already set the
	 * 320x200x8 hardware mode (see compat/sdl2_shim.c); nothing else to do. */
#else
...
#ifndef OF_PC
/* RVSTACK: one fixed display mode — these are inert on the Pocket. */
...
#ifndef OF_PC
// RVSTACK present path: the Pocket framebuffer is 8-bit indexed, exactly like
// the game's VGAScreen, so push the indexed pixels + current palette straight
// to the hardware (RVSDL_PresentIndexed -> sdl_lite SDL_SetColors/SDL_Flip;
// the flip also pumps the audio callback and paces to vsync).
static void of_pocket_present(void)
{
	SDL_Color cols[256];
	get_current_palette(cols);
	RVSDL_PresentIndexed(VGAScreen->pixels, VGAScreen->pitch,
	                     VGAScreen->w, VGAScreen->h, cols);
}
#endif
