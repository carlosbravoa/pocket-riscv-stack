...
#ifndef OF_PC
// Pocket: the framebuffer is 8-bit indexed, so the video path feeds the
// current palette straight to hardware instead of pre-multiplying into RGB.
void get_current_palette(SDL_Color out[256])
{
	for (int i = 0; i < 256; ++i)
		out[i] = palette[i];
}
