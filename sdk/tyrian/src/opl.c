...
/* rv32imafc has a SINGLE-precision FPU only; `double` math is soft-emulated
 * (~10-50x slower) and the per-sample OPL synthesis floods it, starving the
 * whole frame loop (the shim pumps audio synchronously). Use hardware float.
 * (Take care that hot-path literals carry an `f` suffix so the expression
 * doesn't silently re-promote to double.) */
#define fltype float
