/*
 * stdio.h — riscv-stack compat SHADOW header for the OpenTyrian2000 port.
 *
 * This directory is first on the include path, so every TU in the build sees
 * this file instead of picolibc's stdio.h; it chains to the real header via
 * include_next and then re-routes the FILE-stream API the game uses onto
 * compat/rvfile.c (pakfs-backed reads + RAM-staged writes). LiteX's
 * picolibc-minimal has no fopen/fread/... at all, and its fgetc/fputc operate
 * on picolibc's own struct __file — never on our handles — so everything
 * stream-shaped must be intercepted here at compile time.
 *
 * printf/puts/putchar/vfprintf are NOT touched: they are picolibc's and go to
 * the debug UART. stderr/stdout are redefined to sentinels that rvfs_fprintf
 * and rvfs_fputs recognize and forward to the UART via printf.
 *
...
/* picolibc may implement these as macros over its own FILE */
#undef getc
#undef putc
#undef getchar
#undef putchar
 ...
 * provides real definitions matching the prototypes include_next pulled in. */
