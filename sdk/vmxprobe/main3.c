 ...
 *
 * Screen: green = every check matched, red = mismatches (count in the bar
 * width), plus the last pair drawn as two bars. Diag words carry the detail:
 *   0xD0G0xxxx  pass count      0xD0B0xxxx  mismatch count
...
static void bars(uint8_t bg, int ok, int bad, int s0i, int s1i)
{
...
int main(void)
{
	...
	for (;;) {
		...
		while (sys_ticks_us64() - t0 < 2000000) {
			...
			last0 = i0; last1 = i1;
			/* the pair must be two REAL ramp values, adjacent, in order */
			if (i0 >= 0 && i1 >= 0 && (i1 == i0 + 1 || (i0 == N - 1 && i1 == N - 1)))
				ok++;
			else
				bad++;
			bars(phase ? 0xFC : 0x1C, (int)ok, (int)bad, i0, i1);
		}
