...
bool CK_DebugKeys()
{
	...
	if (IN_GetKeyState(IN_SC_G) && game_in_progress)
	{
		...
		if (ck_godMode)
			US_PrintCentered(CK_STRING(ck_str_godModeOff));
		else
	...
	if (IN_GetKeyState(IN_SC_J))
	{
		...
		if (ck_gameState.jumpCheat)
			US_PrintCentered(CK_STRING(ck_str_jumpCheatOn));
		else
	...
	if (IN_GetKeyState(IN_SC_N))
	{
		...
		{
			US_PrintCentered(CK_STRING(ck_str_noClippingOn));
...
void CK_CheckKeys()
{
	...
	if (in_Paused)
	{
		...
		RF_ForceRefresh();
		in_Paused = false;
		in_PausedMessage = CK_STRING(ck_str_paused);
		SD_MusicOn();
	}
	...
	if (!ck_storeDemo)
	{
		...
		if (IN_GetLastScan() == in_kbdControls.quickSave || IN_IsJoyButtonDown(IN_joy_quicksave))
		{
			...
			{
				in_Paused = true;
				in_PausedMessage = CK_STRING(ck_str_gameQuickSaved);
			}
		}
...
void StartMusic(int16_t level)
{
	...
	// NOTE: For buffer overflow emulation in Keen 4,
	// consider assigning in_kbdControls.fire as the song number
#ifdef WITH_KEEN4
	if ((level == -1) && (ck_currentEpisode->ep == EP_CK4))
		song = CK_INT(ck4_specialMusic, 5);
	else
#endif
		song = CK_INTELEMENT(CK_LevelMusic, level);
...
void CK_DrawStatusWindow(void)
{
	...
	{
	case D_Easy:
		US_CPrint(CK_STRING(ck_str_statusEasy));
		break;
	case D_Normal:
		US_CPrint(CK_STRING(ck_str_statusNormal));
		break;
	case D_Hard:
		US_CPrint(CK_STRING(ck_str_statusHard));
		break;
	default:
	...
	while (ck_gameState.levelState == LS_Playing)
	{
		...
		if (ca_mapOn == CK_INT(ck_worldMapNumber, 0))
		{
			...
			if (!ck_mapMiscFlagsCheck)
			{
				switch (ck_currentEpisode->ep)
				{
				// RVSTACK: guard per-episode symbols (only compiled-in
				// episodes exist in this build).
#ifdef WITH_KEEN4
				case EP_CK4:
